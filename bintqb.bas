...
' Almost identical with the FreeBasic version, except for QB64 extensions.
' Compile with qb64 -x -w bintqb.bas
'------------------------------------------------------------------------------------------
' Currently supports:
'
...
'print "docmd"
  ...
  do
    ...
    select case sym
      ...
      case "files":       call getsym: call filesstmt
      case "play":        call getsym: play strexpression$
      case "sound":       call getsym: call soundstmt
      case "_delay":      call getsym: _delay numeric_expr#
      case "_dest":       call getsym: call deststmt
      case "_display":    call getsym: _display
      case "_freeimage":  call getsym: call freeimage
      case "_fullscreen": call getsym: _fullscreen
      case "_limit":      call getsym: call limitstmt
...
sub dimstmt
  ...
  do
    ...
    if ident_type = tystrident then
      ...
      redim _preserve string_arr_store(str_arr_stor_max)
    else
      index = num_arr_stor_max + 1
      num_arr_stor_max = num_arr_stor_max + a_len
      redim _preserve numeric_arr_store(num_arr_stor_max)
    end if
...
sub filesstmt
  dim s as string
  s = ""
  if symtype = tystring or symtype = tystrident then s = strexpression$
  files s
end sub
...
function instrrevfun&
  ...
  expect(")")
  instrrevfun& = _instrrev(i, haystack, needle)
end function
...
function newimagefun&
  dim w as long, h as long, mode as long
  ...
  if accept&(",") then
    mode = numeric_expr#
    newimagefun& = _newimage(w, h, mode)
  else
    newimagefun& = _newimage(w, h)
  end if
  expect(")")
end function
...
function rgbfun~&
  dim r as long, g as long, b as long, h as long
  expect("(")
  r = numeric_expr#
  expect(",")
  g = numeric_expr#
...
function rgb32fun~&
  ...
  if accept&(")") then
    ...
    if accept&(")") then
      ...
      b = numeric_expr#
      if accept&(")") then
        rgb32fun = _rgb32(r, g, b)
      else
        expect(",")
        a = numeric_expr#
        rgb32fun = _rgb32(r, g, b, a)
        expect(")")
      end if
    end if
  end if
...
function rgbafun~&
  dim r as long, g as long, b as long, a as long, h as long
  ...
  if accept&(",") then
    h = numeric_expr#
    rgbafun = _rgba(r, g, b, a, h)
  else
    rgbafun = _rgba(r, g, b, a)
  end if
...
sub printstringstmt
  dim c as integer, r as integer, ex as string
  ...
  ex = strexpression$
  if accept&(",") then
    _printstring (c, r), ex, numeric_expr#
  else
    _printstring (c, r), ex
  end if
end sub
...
function strfactor$
  ...
  select case sym
    ...
    case "_clipboard$": call getsym: strfactor$ = _clipboard$
    case "_cwd$":       call getsym: strfactor$ = _cwd$
    case "_os$":        call getsym: strfactor$ = _os$
    case "_startdir$":  call getsym: strfactor$ = _startdir$
    case "_title$":     call getsym: strfactor$ = _title$
    case "_trim$":      strfactor$ = ltrim$(rtrim$(get_paren_string$))
...
function primary#
  ...
  select case sym
    ...
    case "_atan2": call getsym: primary# = atan2fun#

    case "_ceil": primary# = _ceil(get_paren_numeric#)
    case "_d2g":  primary# = _d2g(get_paren_numeric#)
    case "_d2r":  primary# = _d2r(get_paren_numeric#)

    case "_desktopwidth":  call getsym: primary# = _desktopwidth
    case "_desktopheight": call getsym: primary# = _desktopheight
    case "_fontwidth": call getsym: primary# = _fontwidth
    case "_fontheight": call getsym: primary# = _fontheight

    case "_g2d":  primary# = _g2d(get_paren_numeric#)
    case "_g2r":  primary# = _g2r(get_paren_numeric#)
    case "_height"
      call getsym
      if sym = "(" then
    ...
    case "_instrrev": call getsym: primary# = instrrevfun&

    case "_keydown": primary# = _keydown(get_paren_numeric#)

    case "_keyhit": call getsym: primary# = _keyhit

    case "_mousebutton": primary# = _mousebutton(get_paren_numeric#)

    case "_mouseinput": call getsym: primary# = _mouseinput
    ...
    case "_rgb":  call getsym: primary# = rgbfun
    case "_rgba": call getsym: primary# = rgbafun
    case "_rgba32":call getsym: primary# = rgba32fun
    case "_rgb32":call getsym: primary# = rgb32fun

    case "_round": primary# = _round(get_paren_numeric#)
